...
<body>
  ...
  <div id="password-overlay" class="password-overlay" style="display: none;">
    <div class="password-card" id="password-card">
      <div class="password-title">🔒 가계부 대시보드 보호</div>
      <div class="password-subtitle">비밀번호를 입력하세요<br><small style="color: var(--theme-amber); font-weight: 600;">(힌트 : 결혼기념일 8자리)</small></div>
      <div class="password-input-group">
        <input type="password" id="password-input" class="form-control password-input" placeholder="••••••••" maxlength="8" autofocus onkeydown="if(event.key==='Enter') verifyPassword()" />
        <button class="btn btn-primary" style="justify-content: center; width: 100%;" onclick="verifyPassword()">대시보드 열기</button>
      </div>
      <div id="password-error" class="password-error">비밀번호가 올바르지 않습니다.</div>
    </div>
  </div>
    ...
    <header>
      ...
      <div class="header-actions">
        <label for="file-input" class="btn btn-secondary">
          <span class="upload-icon-small">📁</span> 가계부 파일 업로드 (.csv, .xlsx)
        </label>
  ...
  <script>
    ...
    window.onload = function() {
      const authenticated = sessionStorage.getItem('ledger_authenticated');
    ...
    function verifyPassword() {
      ...
      if (!pwInput || !errorDiv || !card) return;
      ...
      if (input === '20250405') {
        ...
        // reflow to trigger animation restart
      ...
      window.addEventListener('drop', (e) => {
        ...
        if (e.dataTransfer.files.length > 0) {
          ...
          if (filename.endsWith('.csv')) {
            ...
            alert('CSV 또는 Excel(.xlsx) 형식의 파일만 업로드할 수 있습니다.');
          }
        }
      });
      ...
      reader.onload = function(e) {
        try {
          ...
          headers.forEach((h, idx) => {
            if (h) colIndex[h.toString().trim()] = idx;
          });
          
          if (colIndex['날짜'] === undefined || colIndex['금액'] === undefined) {
            alert('필수 열(날짜, 금액)이 누락되었습니다. 엑셀의 열 이름을 확인해주세요.');
            return;
          }
          ...
          function formatDate(val) {
            ...
            if (typeof val === 'string') {
              const clean = val.replaceAll('/', '-').trim();
          ...
          function parseExcelAmount(val) {
            ...
            const amtStr = String(val || '0').replace(/,/g, '').trim();
            let amt = parseFloat(amtStr);
            return isNaN(amt) ? 0.0 : amt;
          }
          ...
          for (let i = 1; i < rows.length; i++) {
            ...
            const amt = parseExcelAmount(getValue('금액', 0));
            parsedData.push({
              date: formatDate(getValue('날짜')),
              time: formatTime(getValue('시간')),
              type: String(getValue('타입', amt < 0 ? '지출' : '수입')),
              category: String(getValue('대분류', '미분류')),
              subcategory: String(getValue('소분류', '미분류')),
              content: String(getValue('내용')),
              amount: amt,
              currency: String(getValue('화폐', 'KRW')),
              payment: String(getValue('결제수단')),
              memo: String(getValue('메모'))
            });
          }

          if (parsedData.length === 0) {
            alert('가져올 가계부 거래 데이터가 없습니다.');
            return;
          }
          ...
          alert('엑셀 파싱 도중 에러가 발생했습니다: ' + err.message);
        }
      };
